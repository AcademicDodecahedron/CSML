...
class SqltypedExtension(Extension):
    """Wraps all expressions with the `sqltyped` filter,

    so `{{ variable | filter1 | filter2 }}`

    becomes `{{ (variable | filter1 | filter2) | sqltyped }}`
